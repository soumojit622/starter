  ...
  return (
    <section className="sticky top-0 flex flex-col gap-10 bg-white border-r px-5 py-3 h-screen overflow-hidden w-[260px] z-50">
      <div className="flex justify-center py-4">
        <Link href={`/`}>
          {/* Adjusted width and height for better sizing */}
          <Image
            className="h-8"
            src="/logo.png"
            alt="Logo"
      ...
      <div className="flex justify-center">
        <button
          ...
          className="flex gap-2 items-center px-3 py-2 hover:bg-indigo-100 rounded-xl w-full justify-center transition-all"
        >
...
function Tab({ item }) {
  ...
  useEffect(() => {
    setIsSelected(pathname === item?.link);
  }, [pathname, item?.link]);
